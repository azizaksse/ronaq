...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex flex-1 items-center justify-between gap-3 px-6 py-4 text-right text-base font-medium text-white transition hover:text-mint",
        className,
      )}
      {...props}
    >
      {children}
      <ChevronDown className="h-4 w-4 shrink-0 transition duration-200 group-data-[state=open]:rotate-180" />
    </AccordionPrimitive.Trigger>
